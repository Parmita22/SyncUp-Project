...
export default function ReleasesUI() {
  ...
  useEffect(() => {
    const fetchVersionData = async () => {
      const versionDataResult = await versionData(organization)
      setAllVersions(versionDataResult)
    }
    fetchVersionData()
    setIsVersionCreated(false)
  }, [isVersionCreated, organization])
            ...
            >
              ...
              >
                <DropdownItem
                  key="Released"
                  className="px-2 py-1"
                  onPress={() => handleStatusFilter("Released")}
                >
                  Released
                </DropdownItem>
                <DropdownItem
                  key="Unreleased"
                  className="px-2 py-1"
                  onPress={() => handleStatusFilter("Unreleased")}
                >
                  Unreleased
                </DropdownItem>
              </DropdownMenu>
